...
public class TestRuleBook
{
    ...
    @Test public void TestFirstLastTonic()
    {
        Bar bar = new Bar(3);
            ...
            bar.addNote(new Note(72, 1));
        p.addBar(bar);
        assertEquals("Testing Rules", rb.firstLastTonic(p), 100);
        p.getNote(p.size()-1).setPitch(73);
        assertEquals("Testing Rules", rb.firstLastTonic(p), 0);
    ...
    @Test public void TestintervalPreferences()
    {
        Bar bar = new Bar(2);
            ...
            bar.addNote(new Note(72, 1));
        p.addBar(bar);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 4);
        p.getNote(p.size()-1).setPitch(73);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 4);
        p.getNote(p.size()-1).setPitch(74);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 9);
        p.getNote(p.size()-1).setPitch(75);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 6);
        p.getNote(p.size()-1).setPitch(76);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 6);
        p.getNote(p.size()-1).setPitch(77);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 3);
        p.getNote(p.size()-1).setPitch(78);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 0);
        p.getNote(p.size()-1).setPitch(79);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 3);
        p.getNote(p.size()-1).setPitch(80);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 2);
        p.getNote(p.size()-1).setPitch(81);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 2);
        p.getNote(p.size()-1).setPitch(82);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 1);
        p.getNote(p.size()-1).setPitch(83);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 1);
        p.getNote(p.size()-1).setPitch(84);
        assertEquals("Testing Rules", rb.intervalPreferences(p), 5);
    ...
    @Test public void TestContourEnforcement()
    {
        Bar bar = new Bar(3);
            ...
            bar.addNote(new Note(72, 1));
        p.addBar(bar);
        assertEquals("Testing Rules", rb.contourEnforcement(p), 5);
        p.getNote(1).setPitch(73);
        assertEquals("Testing Rules", rb.contourEnforcement(p), 0);
    ...
    @Test public void TestFinality()
    {
        ...
        p.lastBar().addNote(new Note(72, 1));
        assertEquals("Testing Rules", rb.finality(p), 0);
        p.addBar(new Bar(2));
        p.lastBar().addNote(new Note(72, 1));
        assertEquals("Testing Rules", rb.finality(p), 0);
        p.addBar(new Bar(2));
        p.lastBar().addNote(new Note(72, 2));
        assertEquals("Testing Rules", rb.finality(p), 100);
    }
    ...
    @Test public void TestKeySignatureEnforcement()
    {
        p.addBar(new Bar(13));
        p.lastBar().addNote(new Note(72, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 7);
        p.lastBar().addNote(new Note(73, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 7);
        p.lastBar().addNote(new Note(74, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 14);
        p.lastBar().addNote(new Note(75, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 14);
        p.lastBar().addNote(new Note(76, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 21);
        p.lastBar().addNote(new Note(77, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 28);
        p.lastBar().addNote(new Note(78, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 28);
        p.lastBar().addNote(new Note(79, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 35);
        p.lastBar().addNote(new Note(80, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 35);
        p.lastBar().addNote(new Note(81, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 42);
        p.lastBar().addNote(new Note(82, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 42);
        p.lastBar().addNote(new Note(83, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 49);
        p.lastBar().addNote(new Note(84, 1));
        assertEquals("Testing Rules", rb.keySignatureEnforcement(p), 56);
    }
    ...
    @Test public void TestSimilarBarStructre()
    {
        ...
        p.getBar(0).addNote(new Note(72, 1));
        p.getBar(0).addNote(new Note(72, 1));
        assertEquals("Testing Rules", rb.similarBarStructure(p), 0);
        p.addBar(new Bar(3));
        p.getBar(1).addNote(new Note(72, 1));
        p.getBar(1).addNote(new Note(72, 1));
        p.getBar(1).addNote(new Note(72, 1));
        assertEquals("Testing Rules", rb.similarBarStructure(p), 5);
        p.addBar(new Bar(3));
        ...
        assertEquals("Testing Rules", rb.similarBarStructure(p), 10);
    }
    ...
    @Test public void TestnoteDuration()
    {
        ...
        //Print.a(""+RuleBook.noteDuration(p));
        assertEquals("Testing Rules", rb.noteDuration(p), 25);
    }
}
